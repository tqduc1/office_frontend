...
/*
  This contains logic for providing a Zoom-like interaction to any DOM node.
  It allows a user to pinch, zoom, translate, etc, as they would an interactive Zoom.
...
*/
export class ZoomInteractionControlled extends Component {
  ...
  */
  ...
  onMouseDown(e) {
    e.preventDefault();
    if (e.target.id === ID_IMAGE_MAP) this.setPointerState([e]);
  }
  ...
  onWheel(e) {
    ...
    if (e.ctrlKey) {
      if (setScale) {
        setScale(this.props.value.scale + (1 - scaleChange));
      }
  ...
  render() {
    ...
    /*
      ...
      component to decide if elements inside the Zoom are clickable. Normally, you wouldn't
...
*/
class ZoomInteractionController extends Component {
  static get propTypes() {
    return {
      children: PropTypes.func,
      value: PropTypes.shape({
  ...
  constructor(props) {
    super(props);
    const controlled = ZoomInteractionController.isControlled(props);
    if (controlled) {
      this.state = {
        lastKnownValueFromProps: props.value,
      };
    } else {
      // Set the necessary state for controlling Zoom interaction ourselves
      this.state = {
  ...
  */
  static getDerivedStateFromProps(props, state) {
    const nowControlled = ZoomInteractionController.isControlled(props);
    const wasControlled =
      state.lastKnownValueFromProps &&
      ZoomInteractionController.isControlled({
  ...
  getValue() {
    const controlled = ZoomInteractionController.isControlled(this.props);
    return controlled ? this.props.value : this.state.value;
  }
  componentDidUpdate(prevProps, _) {
    if (prevProps.rect !== this.props.rect && this.props.rect?.width !== 0) {
      this.setState({
        value: {
          scale: 1,
          translation: {
            x:
              this.props.rectControlZone?.width / 2 -
              this.props.rect?.width / 2,
            y:
              this.props.rectControlZone?.height / 2 -
              this.props.rect?.height / 2,
          },
        },
      });
    }
...
export default ZoomInteractionController;
