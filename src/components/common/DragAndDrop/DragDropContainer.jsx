...
class DragDropContainer extends React.Component {
    ...
    if (this.currentTarget !== this.prevTarget) {
      if (this.prevTarget) {
        this.prevTarget.dispatchEvent(
          this.buildCustomEvent(`${prefix}DragLeave`)
        );
      }
      if (this.currentTarget) {
        this.currentTarget.dispatchEvent(
          this.buildCustomEvent(`${prefix}DragEnter`)
        );
      }
    }
  ...
  handleMouseDown = (e) => {
    if (usesLeftButton(e) && !this.props.noDragging) {
      ...
      if (this.props.type) {
        ...
        x = scale === 1 ? e.clientX + rect.left : 0;
        y = scale === 1 ? e.clientY + rect.top : 0;
      }
      this.startDrag(x, y);
    }
  };
  ...
  startDrag = (clickX, clickY) => {
    document.addEventListener(
      ...
      this.props.id ? this.props.onDrop : () => true
    );
  ...
  drag = (x, y) => {
    ...
    if (offScreen) {
      ...
      if (!this.props.yOnly) {
        stateChanges.left =
          this.props.dragData.id === -1
            ? this.state.leftOffset + x - this.fixedOffsetLeft
            : (x - rect.left - paddingOffset) / scale;
      }
      if (!this.props.xOnly) {
        stateChanges.top =
          this.props.dragData.id === -1
            ? this.state.topOffset + y - this.fixedOffsetTop
            : (y - rect.top - paddingOffset * 2) / scale;
      }
    }
  ...
  getDisplayMode = () => {
    if (
      this.state.dragging &&
      !this.props.dragClone &&
      !this.props.customDragElement
    ) {
  ...
  render() {
    ...
    if (this.props.visible) {
      return (
        <div className="ddcontainer" style={containerStyles}>
          <span className="ddcontainersource" style={sourceElemStyles}>
            {content}
          </span>
        </div>
      );
